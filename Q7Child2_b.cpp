...
uint8_t b = 18;
int 	i = 2 ;				// a simple int
int 	array1[ 10 ]  ; 		// array of 10 integers

char	name[15 ] = "child 2 string" ;			// a string of 15 characters
...
int	main()
{
	...
	pipe2.Write(&i, sizeof(i));				// Read the int 'i' from the pipe
	printf("Child Process 2 wrote integer = %d into Pipeline 2.....\n", i);

	pipe2.Write(&name[0], sizeof(name));
	printf("Message from child 2 is: %s.....\n", name);
