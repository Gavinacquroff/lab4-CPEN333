...
int 	i = 3;				// a simple int
int 	array1[ 10 ]  ; 		// array of 10 integers

char	name[15 ] = "child 3 string";			// a string of 15 characters
...
int	main()
{
	...
	CPipe	pipe3("MyPipe3", 1024);		// Create a pipe 'p1' with the name "MyPipe"

	pipe3.Write(&i, sizeof(i));				// Read the int 'i' from the pipe
	printf("Child Process 3 wrote integer = %d into Pipeline 3.....\n", i);

	pipe3.Write(&name[0], sizeof(name));
	printf("Message from child 3 is: %s.....\n", name);
